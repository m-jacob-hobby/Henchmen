...
void UMainMenuWidget::NativeConstruct() {
	...
	if (GameInstance != nullptr) {
		...
		if (HenchmenGameInstance != nullptr) {
			AccessToken = HenchmenGameInstance->AccessToken;
		}
	}
			...
			if (JsonObject->HasField("access_token") && JsonObject->HasField("id_token") && JsonObject->HasField("refresh_token")) {
				...
				if (GameInstance != nullptr) {
					...
					if (HenchmenGameInstance != nullptr) {
						...
						HenchmenGameInstance->SetCognitoTokens(AccessToken, IdToken, RefreshToken);
...
void UMainMenuWidget::OnMatchmakingButtonClicked() {
	...
	if (GameInstance != nullptr) {
		...
		if (HenchmenGameInstance != nullptr) {
			AccessToken = HenchmenGameInstance->AccessToken;
			MatchmakingTicketId = HenchmenGameInstance->MatchmakingTicketId;
		}
	}
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject)) {
			if (JsonObject->HasField("ticketId")) {
				...
				if (GameInstance != nullptr) {
					UHenchmenGameInstance* HenchmenGameInstance = Cast<UHenchmenGameInstance>(GameInstance);
					if (HenchmenGameInstance != nullptr) {
						HenchmenGameInstance->MatchmakingTicketId = MatchmakingTicketId;
...
void UMainMenuWidget::OnStopMatchmakingResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	...
	if (GameInstance != nullptr) {
		UHenchmenGameInstance* HenchmenGameInstance = Cast<UHenchmenGameInstance>(GameInstance);
		if (HenchmenGameInstance != nullptr) {
			HenchmenGameInstance->MatchmakingTicketId = "";
		}
	}
